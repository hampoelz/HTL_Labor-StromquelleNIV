...
Datenblatt: \textbf{$U_{O_{OPV}} \approx \sage{norm(U_O_OPV)}$}
...
\begin{align*}
    ...
    U_{a0_{max}} &= \sage{norm(U_Bat_OPV)} - \sage{norm(U_O_OPV)} \\
    U_{a0_{max}} &= \sage{norm(U_a0_max)}V
\end{align*}
...
\begin{align*}
    ...
    U_{a_{max}} &= \sage{norm(U_a0_max)} - \sage{norm(U_e0)} \\
    U_{a_{max}} &= \sage{norm(U_a_max)}V
\end{align*}
