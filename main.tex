...
Angaben: \textbf{$I_a = \sage{norm(I_a*1000)}mA$}, $U_{Bat_{OPV}} = \pm \sage{U_Bat_OPV}V$, $U_g = U_{e0} = R_g \cdot I_a$, $OPV = OP27$
...
Die maximale Ausgangsspannung der Schaltung $U_{a0_{max}}$ wird von der Versorgungsspannung des Operationsverstärkers $U_{Bat_{OPV}}$ begrenzt:
...
\begin{equation*}
    U_{a0_{max}} = U_{Bat_{OPV}} = \sage{U_a0_max}V
\end{equation*}
...
\begin{align*}
    ...
    U_{a_{max}} &= \sage{U_a0_max} - \sage{U_e0} = \sage{U_a_max}V
\end{align*}
...
\begin{align*}
    ...
    R_{L_{max}} &= \frac{\sage{U_a_max}}{\sage{norm(I_a)}} \\
...
\begin{align*}
    ...
    R_g &= \frac{U_{e0}}{I_a} \\
    R_g &= \frac{\sage{U_e0}}{\sage{norm(I_a)}} \\
